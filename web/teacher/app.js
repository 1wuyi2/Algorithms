...
async function loadBackendData() {
  ...
  try {
    const coursesRes = await fetch(`${API_BASE_URL}/courses?semester=${semester}`);
    const coursesResult = await coursesRes.json();
    if (!coursesRes.ok || coursesResult.success === false) throw new Error("课程加载失败");
    const backendCourses = coursesResult.data.map(mapBackendCourseToTeacher);
    const teachersRes = await fetch(`${API_BASE_URL}/teachers`);
    const teachersResult = await teachersRes.json();
    if (!teachersRes.ok || teachersResult.success === false) throw new Error("教师加载失败");
    const backendTeachers = teachersResult.data.map(t => ({
      id: t.name,
      name: t.name,
      college: t.college,
    ...
    const roomsRes = await fetch(`${API_BASE_URL}/classrooms`);
    const roomsResult = await roomsRes.json();
    if (!roomsRes.ok || roomsResult.success === false) throw new Error("教室加载失败");
    const backendRooms = roomsResult.data.map(r => ({
      id: r.name,
      name: r.name,
      capacity: r.capacity,
      campus: r.campus,
    ...
    state.teachers = backendTeachers;
...
function mapBackendCourseToTeacher(row) {
  return {
    ...
    classGroupIds: ["DEFAULT"],
    weeklyHours: row.weekly_hours || 3,
    expectedStudents: row.quota,
    requiredRoomType: "general",
    requiredCampus: row.campus === "津南" ? "jinnan" : row.campus === "八里台" ? "balitai" : null,
    fixedTimeSlotId: null,
    candidateTimeSlotIds: []
  };
}